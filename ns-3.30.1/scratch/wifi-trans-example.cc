...
int main (int argc, char** argv)
{
  ...
  /* nodes and positions */
  uint16_t  NumOfNodes = 10;
  NodeContainer wifiNodes;
  NodeContainer spectrumAnalyzerNodes;
  NodeContainer allNodes;
  wifiNodes.Create (NumOfNodes);
  ...
  for (uint16_t u = 1; u < NumOfNodes; ++u )
  {
	wifiStaNode.Add (wifiNodes.Get (u));
  }
  ...
  for (uint32_t i = 0; i < wifiNodes.GetN (); ++i)
    {
      anim.UpdateNodeDescription (wifiNodes.Get (i), "UE"); // Optional
      anim.UpdateNodeColor (wifiNodes.Get (i), 255, 0, 0); // Optional
    }
  for (uint32_t i = 0; i <  spectrumAnalyzerNodes.GetN (); ++i)
    {
      anim.UpdateNodeDescription (spectrumAnalyzerNodes.Get (i), "Remote Host"); // Optional
      anim.UpdateNodeColor (spectrumAnalyzerNodes.Get (i), 0, 0, 255); // Optional
    }
